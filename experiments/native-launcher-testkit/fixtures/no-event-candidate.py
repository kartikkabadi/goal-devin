#!/usr/bin/env python3
"""Negative-control candidate that writes a valid manifest/record/summary with
``total_events: 1`` but never publishes an event.  The shared runner must reject it.
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "run_id": run_id,
        "created_at": observed,
        "command": f"no-event-candidate --model {args.model} --permission-mode {args.permission_mode}",
        "model": args.model,
    ...
    # Intentionally no event file in events/.
    print("Goal Devin native mode completed (events=0)")
