#!/usr/bin/env python3
"""Negative-control candidate that deletes a pre-existing canary file during an
otherwise valid run and exits zero. The shared runner must detect the deletion.
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "run_id": run_id,
        "created_at": observed,
        "command": f"delete-preexisting-success-candidate --model {args.model} --permission-mode {args.permission_mode}",
        "model": args.model,
        "permission_mode": args.permission_mode,
        "devin_bin": str(Path(args.devin_bin).resolve()),
        "canary": str(canary.resolve()),
        "hook_command": ["python3", str(run_dir / "hook")],
    ...
    record = {
        ...
        "permission_mode": args.permission_mode,
        "profile_id": profile_id,
        "hook_command": manifest["hook_command"],
        "hook_returncode": 0,
    ...
    for name in ("event.schema.json", "limits.schema.json"):
        src = contract_dir / "expected" / name
        dst = run_dir / name
        if src.exists():
            dst.write_text(src.read_text(encoding="utf-8"), encoding="utf-8")
            os.chmod(dst, 0o600)

    limits_src = contract_dir / "limits.json"
    ...
    # Clean up run-owned artifacts before deleting the pre-existing canary file.
    ...
    if agents_parent.exists() and not any(agents_parent.iterdir()):
        try:
            agents_parent.rmdir()
        except OSError:
            pass

    # Delete a pre-existing canary file; the runner must detect this.
